        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    if c == 16 or c == 22:
                        twenty = n * 20
                    elif int(c) == 15 or int(c) == 18:
                        handred = n * 100
                    elif c == 17 or c == 21:
                        fifty = n * 50
                    elif c == 19 or c == 20:
                        twohandred = n * 200
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)} Detected, "  # add to string
            ...
            sum1 = twenty + handred + fifty + twohandred
            print(f'{sum1}')
            # Stream results
            if view_img:
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    textsize = cv2.getTextSize(f'amount={sum1}', font, 1, 2)[0]
                    # get coords based on boundary
                    textX = round((im0.shape[1] - textsize[0]) / 2)
                    textY = round((im0.shape[0] + textsize[1]) / 2)
                    cv2.putText(im0, f'amount={sum1}', ((textX), (textY)),
